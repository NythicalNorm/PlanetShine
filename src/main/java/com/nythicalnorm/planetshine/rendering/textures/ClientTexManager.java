...
public class ClientTexManager {
    ...
    public void incomingPlanetTexture(ClientCelestialBody planet, byte[] tex) {
        ...
        try {
            ...
            ResourceLocation texResourceLocation = texturemanager.register("planetshine/planet_tex/" + planet.getName(), texture);
            planet.setMainTexture(texResourceLocation);
        } catch (IOException e) {
            PlanetShine.logError(e.toString());
            PlanetShine.logError("png texture can't be parsed");
        }
    }
    ...
    private boolean loadTextureToBiomeAtlas(byte[] tex, Vector2i texPos) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            PlanetShine.getLogger().error("can't parse biome texture.");
            e.printStackTrace();
        }
        return false;
    }
